...
data = subprocess.check_output(['netsh', 'wlan', 'show', 'profiles']).decode('Windows-1251').split('\n')
...
for i in profiles:
    ...
    results = subprocess.check_output(['netsh', 'wlan', 'show', 'profile', i, 'key=clear']).decode('Windows-1251').split('\n')
